import { CountryFillLayer, type CountryFillLayerOptions } from '../shared/country-fill-layer';

export type {
  CountryFillLayerOptions as PaperCountryFillLayerOptions,
  CountryFillMode,
} from '../shared/country-fill-layer';

export class PaperCountryFillLayer extends CountryFillLayer {
  public constructor(options: CountryFillLayerOptions) {
    super(options);
    this.group.name = 'PaperCountryFillLayer';
  }
}
