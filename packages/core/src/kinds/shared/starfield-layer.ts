...
export class StarfieldLayer {
  ...
  /**
   * Blending mode for the star sprites. `AdditiveBlending` reads as light
   * against a dark sky; kinds that want the points to sit like pigment on a
   * lit stage override this (paper uses `NormalBlending`). A prototype hook
   * rather than a constructor option so `KindLayerRegistry`'s constructor
