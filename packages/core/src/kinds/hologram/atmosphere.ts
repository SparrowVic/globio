...
export interface HologramAtmosphereOptions {
...
const resolveSide = (mode: HologramAtmosphereOptions['side']): Side => {
  if (mode === 'front') return FrontSide;
  if (mode === 'double') return DoubleSide;
  return BackSide;
};

const resolveBlending = (mode: HologramAtmosphereOptions['blending']): Blending => {
  if (mode === 'normal') return NormalBlending;
  return AdditiveBlending;
};

export class HologramAtmosphereLayer {
  ...
  public constructor(options: HologramAtmosphereOptions) {
  ...
  public setSide(mode: HologramAtmosphereOptions['side']): void {
    this.material.side = resolveSide(mode);
    this.material.needsUpdate = true;
  }

  public setBlending(mode: HologramAtmosphereOptions['blending']): void {
   ...
   */
  public setPulse(pulse: HologramAtmosphereOptions['pulse'] | null): void {
