...
export const buildFaceStartTimes = (
  ...
  const indices = includedIndices(faceCount, included);
  out.fill(delay);
  if (indices.length === 0) return;
  if (stagger <= 0 || order === 'sequential') {
    for (let i = 0; i < indices.length; i++) out[indices[i]!] = delay + i * stagger;
    return;
  }

  const rank = new Float32Array(indices.length);
  if (order === 'radial') {
    fillRadialRank(indices, faceLatLng, origin, rank);
  } else if (order === 'random') {
    fillRandomRank(indices.length, rank);
  } else {
    fillValueRank(indices, values, order === 'value' ? 'desc' : 'asc', rank);
  }
  for (let i = 0; i < indices.length; i++) {
    out[indices[i]!] = delay + rank[i]! * stagger;
  }
};

const includedIndices = (faceCount: number, included: Uint8Array | undefined): number[] => {
  const out: number[] = [];
  for (let f = 0; f < faceCount; f++) {
    if (!included || included[f] !== 0) out.push(f);
  }
  return out;
};
...
const fillValueRank = (
  ...
  const finite = new Float32Array(indices.length);
  for (let i = 0; i < indices.length; i++) {
    const f = indices[i]!;
    const v = values[f]!;
    finite[i] = Number.isFinite(v) ? v : direction === 'asc' ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY;
  }
