...
- **Hex-bin aggregation** `[v1·LAYER·M·built]` 🌟 — `globe.setDataLayer({ type: 'hexbin', data, resolution?, aggregate?, height?, scale?, animation? })`. Lat/lng point samples binned into faces of a subdivided icosphere — visually triangular cells (the "hex" name follows the geographic-binning convention; goldberg-polyhedron real-hexagon variant is a future upgrade). Subdivision levels 0..5 (20 → 20480 cells, default 3 = 1280); 4 aggregate modes (`sum / count / mean / max`); per-face value drives both colour (via `scale`) and outward extrusion (`height: { min, max }`). Cells render as vertex-coloured `BufferGeometry` triangles, each face with its own 3 unshared vertices so colours don't bleed into neighbours. Animation reuses the heatmap easing library — per-face stagger by `cell index × stagger` produces a wave bloom across the globe. Bake is single-pass nearest-centroid lookup (10k samples × 1280 cells ≈ 30ms). Module: `data-layers/hexbin/` split into `icosphere / aggregator / hexbin-mesh / hexbin-layer`. Demo: `examples/vanilla-demo/hexbin.html` (4 datasets — random 2k/10k, 3-region clusters, latitude bands). Decoration: outline kind only v1.
- **Charts (multi-series, anchored)** `[v1·LAYER·M·built]` 🌟 — `globe.setDataLayer({ type: 'charts', data, chartType, series, scale?, size?, height?, innerRadius?, padAngle?, animation? })`. 5 sub-types: `'bars-grouped'` (parallel bars side-by-side), `'bars-stacked'` (single column with composition segments), `'pie'` (flat disc), `'donut'` (pie with hollow centre), `'radial'` (bars arranged around a circle). Each chart anchored at lat/lng or country centroid (resolved via `id` matching the active kind's feature index — same pattern as Bars). Per-series colour wins over the layer-level `scale`; pie/donut billboard toward the camera so they stay readable at high latitudes. Animation reuses the heatmap easing library (rise/pop/fade × 33 curves) — per-chart stagger via `index × stagger`. CPU-driven (10²–10³ instances cheap, no shader pipeline needed). Module: `data-layers/charts/` split into `anchors / bars-builder / pie-builder / charts-layer`. Demo: `examples/vanilla-demo/charts.html` (3 datasets — G7 energy mix, population age, quarterly sales). Decoration: outline kind only v1. **TODO / potencjał:** per-segment reveal in stacked/radial (today: whole stack scales as one), value labels overlay (`showValues` is a stub), per-series scale axes for radial, link arcs between chart anchors, click events with payload, port to dotted/hologram kinds (additive glow / scanline variants).
...
| **v0.55** *(in progress)* | Data layers polish | Shader-based heatmap (5 kernels, country domes, grid+contour overlays, animation system with 33 easings + per-pixel delay map), **multi-series charts (5 sub-types: grouped/stacked bars, pie, donut, radial)**, **hex-bin spatial aggregation (icosphere binning, 4 aggregate modes)**, 4 standalone demo pages |
