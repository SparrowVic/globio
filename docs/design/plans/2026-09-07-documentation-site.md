...
**Status:** skeleton merged to `main` on 2026-09-07 (component library, manifest, route, landing links). Phase 1 executed the same day on `feat/docs-phase-1`: the Vue and Angular wrappers now forward the whole `GlobeConfig` and every event (`GLOBE_CONFIG_KEYS` + `pickGlobeConfig` in the core), `examples/vanilla-demo/scripts/docs-extract.mjs` writes `src/docs/generated/api.json` (548 config keys, 43 methods, 10 events, 60 types, wrapper surfaces) with a vitest that fails when it is stale, the feature registry lives in `src/docs/features/` (44 features, coverage-tested), and every page in the manifest has real content generated from `api.json` or written by hand. Phase 2 followed on `feat/docs-phase-2`: `FeatureTip` (a hover card with the registry summary, the config key's type, default and JSDoc from `api.json`, an optional illustrated tip and a docs link) renders from `ControlLabel` for every Studio control; controls name a `configPath` or `feature`, panel sections and workshop presets provide a `FeatureScope`, ten features carry illustrated tips, and a vitest parses the Studio sources to prove coverage. Phase 3 followed on `feat/docs-phase-3`: a Recipes tab with five complete builds whose live previews run the same data as the snippets (`LivePreview` gained a `setup` hook), ⌘K search over pages, features, config keys, methods and events with exact-match ranking and deep links to table rows (`src/docs/search-index.ts`), an edit-on-GitHub link on every page (page sources live in the page registry), and kind thumbnails captured with `toImage()` on the hidden `/docs/_snapshots` route and saved under `public/docs/kinds/`. Not done: per-version docs (no release yet).
